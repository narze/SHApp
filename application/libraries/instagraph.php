...
class Instagraph
{
    ...
    public function __construct($image = NULL, $output = NULL, $temp_path = NULL) {
        // if($image && $output && $temp_path) {
        //     $this->init($image, $output, $temp_path);
        // }
    }

    public function init($image, $output, $temp_path)
    {
    ...
    public function tempfile()
    {
        ...
        $this->_tmp = $this->_temp_path.$this->_prefix.rand();
    ...
    public function execute($command)
    {
        ...
        echo $command = str_replace(array("\n", "'"), array(' ', '"'), $command);
        // $command = escapeshellcmd($command);
        # execute convert program
        echo passthru($command, $return);
        echo '<pre>';
        var_dump($return);
        echo '</pre>';
    }
    ...
    {
        $this->execute("convert -verbose $input -bordercolor $color -border {$width}x{$width} $input");
    }

    public function frame($input, $frame)
    {
        $this->execute("convert -verbose $input ( '$frame' -resize {$this->_width}x{$this->_height}! -unsharp 1.5×1.0+1.5+0.02 ) -flatten $input");
    }
    ...
    {
        $this->tempfile();
        $this->execute("convert -verbose $this->_tmp -modulate 120,10,100 -fill '#222b6d' -colorize 20 -gamma 0.5 -contrast -contrast $this->_tmp");
    ...
    public function toaster()
    {
        ...
        $this->execute("convert -verbose $this->_tmp -modulate 150,80,100 -gamma 1.2 -contrast -contrast $this->_tmp");
    ...
    public function nashville()
    {
        ...
        $this->execute("convert -verbose $this->_tmp -contrast -modulate 100,150,100 -auto-gamma $this->_tmp");
    ...
    public function lomo()
    {
        ...
        $command = "convert -verbose {$this->_tmp} -channel R -level 33% -channel G -level 33% $this->_tmp";
